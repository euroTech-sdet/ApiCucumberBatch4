Feature: Sign Up
  ...
  Scenario Outline: User can able to POST request and gets the token and saves profile
    ...
    Then Verify that status code should be 200
    Then User verify that name as "John Devis" and email as "john123@gmail.com"

    Examples:
      | company | website       | location | status    | skills   | githubUsername | youtube  | twitter      | facebook  | linkedin  | instagram |
      | Apple   | www.apple.com | New York | Developer | CSS,JAVA | appleGit       | appleYou | appleTwitter | appleFace | appleLink | appleIns  |
